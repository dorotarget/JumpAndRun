...
public class Gegner2 extends Gegner
{
    ...
    public Gegner2(PlayScreen screen, float x, float y) {
        ...
        frames.add(new TextureRegion(screen.getFahrzeugeAtlas().findRegion("auto"), 1, 1, 510, 160));

        laufAnimation = new Animation(0.4f, frames);
        statusZeit = 0;
        setBounds(getX(), getY(), 90 / JumpAndRun.PPM, 30 / JumpAndRun.PPM);
        setToDestroy = false;
        tempo = new Vector2(-1.35f , 0);
        destroyed = false;
   ...
   //     shape.setRadius(6 / JumpAndRun.PPM);
        ...
        vertice[1] = new Vector2(31, 4.5f).scl(1 / JumpAndRun.PPM);
        vertice[2] = new Vector2(-42, -8.5f).scl(1 / JumpAndRun.PPM);
        vertice[3] = new Vector2(31, -8.5f).scl(1 / JumpAndRun.PPM);
